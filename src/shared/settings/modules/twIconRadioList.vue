<template>
    ...
    <IconRadioButton v-if="!defaultRadioBtn.isText" id="twitterIcon.icon" :valueName="defaultRadioBtn.radioName" :name="getSettingI18n('twitterIcon.icon', 'nomal')" :icon="defaultRadioBtn.icon" />
        ...
        <div>
            <div style="display: flex; gap: 10px">
                <template v-for="i in presetsRadioButtonsIconAvailable" :key="i.radioName">
                    <IconRadioButton id="twitterIcon.icon" :valueName="i.radioName" :name="getSettingI18n('twitterIcon.icon', i.radioName)" :icon="i.icon" />
                </template>
            </div>
    ...
    <IconRadioButton v-if="!customRadioBtn.isText" id="twitterIcon.icon" :valueName="customRadioBtn.radioName" :name="getSettingI18n('twitterIcon.icon', 'custom')" :icon="customRadioBtn.icon" />
</template>
...
export interface twIconsRadioListProps {
    ...
    /**
     * ラジオボタンに表示するアイコンの Vue コンポーネントもしくは画像 URL
     */
